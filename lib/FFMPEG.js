...
const FFMPEG = function (hap, cameraName, startFeedSupplier, stopFeedSupplier, imageSupplier, cachedImageSupplier, log) {
    this.log = log;
    this.cameraName = cameraName;
    this.startFeedSupplier = startFeedSupplier;
    this.stopFeedSupplier = stopFeedSupplier;
    this.imageSupplier = imageSupplier;
    this.cachedImageSupplier = cachedImageSupplier;
...
FFMPEG.prototype = {
    async getCameraFeed() {
        let feedSessionInfo = await this.startFeedSupplier();

        this.log.debug('Got sessionInfo=', feedSessionInfo);

        let feed = feedSessionInfo.session.streamHlsUrl;

        if (!feed) {
            throw new Error("Missing source for camera: " + this.cameraName);
        }

        this.log.debug('Feed URL', feed);
    ...
    handleSnapshotRequest(request, callback) {
        ...
        this.imageSupplier()
            .then(response => {
                ...
                let image = response.split(',')[1];
                callback(null, new Buffer(image, 'base64'));
            })
            .catch(() => {
                this.cachedImageSupplier()
                    .then(response => {
                        this.log.debug(response);
                        let image = response.split(',')[1];
                        callback(null, new Buffer(image, 'base64'));
                    ...
                    .catch(error => {
                        this.log.error('Could not get snapshot');
                        callback(error);
                    });
            })
    },
        ...
        if (sessionID) {
            ...
            if (requestType == "start") {
                ...
                if (sessionInfo) {
                    ...
                    this.getCameraFeed()
                        .then(videoSource => {
                            let ffmpegCommand = '-i ' + videoSource +
                                ' -map 0:1' +
                                //' -threads 0' +
                                ' -vcodec ' + videoCodec +
                                // ' -an' +
                        ...
                        .catch(error => this.log.error('Could not establish stream', error));
                }
